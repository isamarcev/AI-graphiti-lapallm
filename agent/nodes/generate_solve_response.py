...
class AgentResponseSchema(BaseModel):
    ...
    references: List[str] = Field(
        description="Список ID джерел (source_uid), інформація з яких була використана. Копіюй ID з атрибутів <source id='...'>. Приклад: ['msg_123', 'doc_5']"
    )
    response: str = Field(
...
4. Сформуй відповідь, яка ТОЧНО відповідає запиту за змістом І форматом.
...
3. **REASONING vs RESPONSE:**
   - `reasoning` — твої внутрішні роздуми (користувач НЕ бачить).
   - `response` — ЄДИНЕ, що побачить користувач. Жодних зайвих слів.

4. **ВІДСУТНІСТЬ ДАНИХ:**
   - Якщо потрібної інформації немає: "На жаль, у мене немає інформації про це."

**ГОЛОВНЕ:** Читай запит користувача ДУЖЕ уважно. Якщо він просить відповідь у певному форматі — дотримуйся його БУКВАЛЬНО.
"""
