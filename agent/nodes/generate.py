...
from agent.helpers import extract_message_uids_from_text
...
async def generate_answer_node(state: AgentState) -> Dict[str, Any]:
    ...
    # Build context з explicit source UIDs (безпосередньо в форматі references)
    retrieved_context = state.get("retrieved_context", [])
    context_parts = []
    source_uid_map = {}  # Мапа UID → context для верифікації
    
    for ctx in retrieved_context:
        source_uid = ctx.get("source_msg_uid", "unknown")
        content = ctx.get("content", "")
        score = ctx.get("score", 0.0)
        
        # Використовуємо UID безпосередньо як reference (не індекси!)
        context_parts.append(
            f"[{source_uid}] (релевантність: {score:.2f}):\n{content}"
        )
        source_uid_map[source_uid] = ctx
...
3. Для КОЖНОГО твердження вказуй ID джерела у квадратних дужках [msg-XXX] (точно як показано в джерелах!)
...
- Кожна використана конструкція = reference [msg-XXX]
...
"Ось структура базуючись на [msg-002]:
[конкретна структура з джерела]

Використав синтаксис з [msg-002]: [пояснення]"
...
Відповідай українською мовою. Обов'язково вказуй джерела у форматі [msg-XXX] точно як вони показані в контексті."""
    ...
    try:
        ...
        # Extract використані source UIDs з тексту (пряме витягування UID!)
        used_source_uids = extract_message_uids_from_text(response_text)
        
        # Verify що всі використані UIDs є в retrieved context
        valid_uids = set()
        for uid in used_source_uids:
            if uid in source_uid_map and uid != "unknown":
                valid_uids.add(uid)
                logger.debug(f"Using source: {uid}")
            else:
